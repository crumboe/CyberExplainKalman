...
(() => {
  const boot = () => document.querySelectorAll("[data-tank-demo]").forEach(root => {
    ...
    let s, paused = false, timer = null, seen = [];
    ...
    function reset() {
      ...
      seen = []; paused = false; get("pause").textContent = "Pause"; draw(); readout(false);
    }
    function update() {
      const m = +get("motion").value, noise = +get("sensor").value, k = +get("gain").value;
      ...
      seen = tags.map((tag,i) => ({...tag,i,d:Math.hypot(tag.x-s.x,tag.y-s.y)}))
        .filter(tag => tag.d <= 4.6 && Math.abs(wrap(Math.atan2(tag.y-s.y,tag.x-s.x)-s.a)) <= Math.PI/5);
      s.t++;
      const corrected = seen.length > 0 && s.t % 4 === 0;
      if (corrected) {
        // Independent camera observations at 5 Hz; physics/prediction at 20 Hz.
        const sigma = noise / Math.sqrt(seen.length), hSigma = sigma * 0.14;
        s.ex += k * (s.x + rand()*sigma - s.ex);
        s.ey += k * (s.y + rand()*sigma - s.ey);
        s.ea = wrap(s.ea + k * wrap(s.a + rand()*hSigma - s.ea));
        s.p = (1-k)**2*s.p + k*k*sigma*sigma;
        s.pa = (1-k)**2*s.pa + k*k*hSigma*hSigma;
      }
    ...
    function readout() {
      ...
      get("status").textContent = paused ? "Paused" : seen.length && +get("gain").value > 0 ? "Wheel prediction + camera correction" : "Predicting from wheels only";
    }
    function draw() {
      ...
      tags.forEach((tag,i) => {
        const x=X(tag.x),y=Y(tag.y),vis=seen.some(t=>t.i===i);
      ...
      ctx.font="18px sans-serif";ctx.fillStyle="#d5e5f2";ctx.textAlign="left";ctx.fillText("12 m × 7 m field     Camera: 72° / 4.6 m",ox,23);
